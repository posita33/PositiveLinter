...
{
	UE_LOG(LinterCommandlet, Display, TEXT("PositiveLinter Usage: {Editor}.exe Project.uproject -run=Linter \"/Game/\""));
	UE_LOG(LinterCommandlet, Display, TEXT(""));
	UE_LOG(LinterCommandlet, Display, TEXT("This will run PositiveLinter on the provided project and scan the supplied directory, example being the project's full Content/Game tree. Can add multiple paths as additional arguments."));
}
...
int32 ULinterCommandlet::Main(const FString& InParams)
{
	...
	UE_LOG(LinterCommandlet, Display, TEXT("PositiveLinter is running."));
